        ...
        {steps.map((step, index) => {
          ...
          return (
            <button
              ...
              className={`flex min-w-0 items-center gap-3 rounded-xl px-3 py-3 text-left transition active:scale-[0.98] ${
                active ? "bg-emerald-50 text-emerald-900" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
              <span className={`grid size-8 shrink-0 place-items-center rounded-lg ${active ? "bg-emerald-700 text-white" : "bg-slate-100 text-slate-500"}`}>
                <Icon className="size-4" />
              </span>
              <span className="min-w-0">
                <span className="block text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-400">Step {index + 1}</span>
